...
void GA::Select() 
{
	...
	for (i = 0; i < PS; i++) 
	{
		fit_sum += (zq[i].fit / 1000) + zq[i].average_affinity;
	}
	//使用轮赌法进行选择
	selected_rate[0] = float((zq[0].fit / 1000) + zq[0].average_affinity) / fit_sum;

	for (i = 1; i < PS; i++) 
	{
		cur_rate = selected_rate[i - 1] + float((zq[i].fit / 1000) + zq[i].average_affinity) / fit_sum;
